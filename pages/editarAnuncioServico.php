...
if(isset($_GET['id_servico'])){
  ...
  $dadosAnuncio = carregarAnuncioServico($conn, $_GET['id_servico'], false);
  ...
  <body>
          ...
          <div class="containerDireita col-8 d-flex flex-column h-100 justify-content-between align-items-center ps-3 pe-3 p-4">
            <div class="row col-12">
              <div class="tituloAnuncio d-flex justify-content-center align-items-center col-12">
                <input type="text" class="form-control text-center" id="iNomeAnuncioServico" name="nNomeAnuncioServico" placeholder="Titulo do Anúncio..." value="<?php echo $dadosAnuncio['nome_servico']?>">
              </div>
            </div>
            <div class="row col-12 d-flex justify-content-between">
              <div class="categoriasServico d-flex flex-column justify-content-center align-items-center col-4">
                ...
                <select type="text" class="form-control text-center disabledBackground" id="iCategoriaServico" name="nCategoriaServico">
                </select>
              </div>
              <div class="duracao d-flex flex-column justify-content-center align-items-center col-4">
                <label for="duracao">Duração:</label>
                <input type="text" class="form-control text-center" id="iDuracaoServico" name="nDuracaoServico" value="<?php echo strval($dadosAnuncio['duracao_servico']).' horas'?>">
              </div>
              <div class="postagem d-flex flex-column justify-content-center align-items-center col-4">
                <label for="postagem">Data de Postagens:</label>
                <input type="text" class="form-control text-center bg-transparent" id="iDataPostagemServico" name="nDataPostagemServico" value="<?php echo date('d/m/Y', strtotime($dadosAnuncio['data_postagem']))?>" disabled>
              </div>
            </div>
            <div class="row col-12">
              <div class="form-floating d-flex flex-column justify-content-center align-items-center col-12">
                <textarea class="form-control textarea" name="nDescricaoAnuncioServico" id="iDescricaoAnuncioServico" ><?php echo $dadosAnuncio['descri_servico'] ?></textarea>
              </div>
            </div>
            <div class="row col-12">
              <div class="form-floating d-flex justify-content-center align-items-center col-12">
                ...
                <button class="btn  col-12 rounded-0 cadastrarAnuncio"><p class="fs-9 m-0 text-white">EDITAR ANÚNCIO</p></button>
              </div>
            </div>
          </div>
        </form>
    ...
    <div class="toast position-fixed bottom-0 end-0 m-2" id="cadastroToast" role="alert" aria-live="assertive" aria-atomic="true" ata-animation="true" data-autohide="true" data-delay="3000" >
      ...
      <div class="toast-body">
        Por favor, preencha os campos: Titulo do Anúncio, Categoria, Duração e Descrição do Anúncio para editar um Anúncio!
      </div>
    </div>
  ...
  jQuery(document).ready(function() {
    ...
    $("#formEditarAnuncio").on('submit', function(e) {
        ...
        if(nomeAnuncioServico != "" && nomeAnuncioServico != null && categoriaAnuncioServico != "" && categoriaAnuncioServico != null 
          ...
          $.ajax({
              method: "POST",
              url: "../config/alterarAnuncioServico.php",
              data: formData,
              processData: false,
              contentType: false, // 
      ...
      $.ajax({
        ...
        success: function(response) {
            ...
            for(let i = 0; i < arrayImagens.length; i++){
                ...
                imagensPreview += `<img class="mx-auto d-block rounded" src="../assets/imagensAnunciosServicos/${arrayImagens[i]}" height="250px" width="250px">`;
                imagensPreview += `</div>`;
