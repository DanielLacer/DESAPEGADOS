...
foreach($_FILES["nImagens"]["name"] as $i=> $name) {
    ...
    if (is_uploaded_file($_FILES['nImagens']['tmp_name'][$i])) {
        ...
        $response['message'] = "Por favor, cadastre pelo menos uma imagem para um novo anúncio de produto.";
        $continue = false;
        break;
    }
...
if($continue){
    ...
    foreach($fotoType as $i => $type){
        ...
        if ((strpos($mime, 'image/') !== 0 || getimagesize($fotoTmpName[$i]) === false) && exif_imagetype($fotoName[$i]) === false) {
            $response['status']  = 'error';
            $response['message'] = "O arquivo enviado para o anuncio não é uma imagem.";
            $continue = false;
            break;
        }  
    ...
    if($continue){
        ...
        if($continue){
            ...
            if($cont > 0){
                ...
                $stmt = $conn->prepare("INSERT INTO produtos (nome_produto, quantidade_produto, descri_produto, id_categoria_produto, fotos_produto, data_postagem, id_usuario) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("sssisss", $nomeAnuncioProduto, $quantidadeProduto, $descricaoAnuncioProduto, $categoriaProduto, $fotosImagens, $dataAtual, $_SESSION['id_usuario']);
