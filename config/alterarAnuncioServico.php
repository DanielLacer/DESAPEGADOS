...
if($_POST["nIdAnuncioServico"] != ""){
    $id_servico = htmlspecialchars($_POST["nIdAnuncioServico"], ENT_QUOTES, 'UTF-8');
}
...
if($continue){
    ...
    if($continue){
        ...
        if($cont > 0 || empty($fotoName)){
            ...
            if(empty($fotoName)){
                $stmt = $conn->prepare("UPDATE servicos SET nome_servico = ?, id_categoria_servico = ?, duracao_servico = ?, descri_servico = ? WHERE id_servico = ? AND id_usuario = ?");
                $stmt->bind_param("ssssss", $nomeAnuncioServico, $categoriaServico, $duracaoServico, $descricaoAnuncioServico, $id_servico, $_SESSION['id_usuario']);
            }else{
                $stmt = $conn->prepare("UPDATE servicos SET nome_servico = ?, id_categoria_servico = ?, duracao_servico = ?, descri_servico = ?, fotos_servico = ? WHERE id_servico = ? AND id_usuario = ?");
                $stmt->bind_param("sssssss", $nomeAnuncioServico, $categoriaServico, $duracaoServico, $descricaoAnuncioServico, $fotosImagens, $id_servico, $_SESSION['id_usuario']);
            }
