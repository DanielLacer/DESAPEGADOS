...
if ($result->num_rows > 0) {
    ...
    if (in_array($remover_categoria, $categorias_servico_existente)) {

        $stmt = $conn->stmt_init();
        $stmt = $conn->prepare("SELECT id_categoria FROM categorias_servico WHERE nome_categoria = ?");
        ...
        if ($result->num_rows > 0) {
            ...
            $stmt = $conn->stmt_init();
            $stmt = $conn->prepare("SELECT * FROM servicos WHERE id_categoria_servico = ?");
            ...
            if ($result->num_rows > 0) {
                ...
                $response['message'] = "A categoria a ser removido já está sendo usado por anúncios de serviços no sistema";

            } else {
               
                $conn->autocommit(FALSE);
                $stmt = $conn->stmt_init();
                $stmt = $conn->prepare("DELETE FROM categorias_servico WHERE nome_categoria = ?");
                $stmt->bind_param("s", $remover_categoria);
